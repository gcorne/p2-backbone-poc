...
( function( $, _, Backbone, p3, app ) {
	...
	var CommentView = React.createBackboneClass({
		...
		render: function() {
			...
			return (
				React.DOM.div(null, 
					React.DOM.div( {className:"avatar", dangerouslySetInnerHTML:{__html: this.props.model.get( 'avatar')}}),
					React.DOM.p( {className:"author"},  this.props.model.get( 'comment_author' ) ),
					React.DOM.p( {dangerouslySetInnerHTML:{__html: this.props.model.get( 'comment_content')}}),
					React.DOM.p( {className:"actions"}, React.DOM.span( {className:"reply", onClick: this.toggleReplyForm }, "reply")),
					 replyForm 
				)
			);
		}
		...
		render: function() {
			...
			if( nodes ) {
				childNodes = nodes.map(function( model ) {
					return ( React.DOM.li( {key:model.get('cid')}, CommentsTreeView( {model: model } )) );
				} );
			}
	...
	var PostView = React.createBackboneClass({
		...
		render: function()  {
			...
			if ( model.get( 'comment_count' ) == 0 ) {
				...
				comments = (
					React.DOM.div(null, 
					React.DOM.span( {className:"new-comment", onClick:this.toggleForm}, "new comment"),
					 commentForm 
					)
				);
			} else {
				comments = (
					React.DOM.div(null, 
					React.DOM.span( {className:"comments", onClick:this.toggleComments},  this.props.model.get( 'comment_count' ),  " comments"),
					React.DOM.div( {className:commentsClass}, CommentsTreeView( {model:this.props.model.comments, isRoot:true} ))
					)
				);
			}
			return (
			React.DOM.div(null, 
			React.DOM.h3(null, React.DOM.a( {href: this.props.model.get( 'permalink' ),  dangerouslySetInnerHTML:{__html: this.props.model.get( 'post_title' ) }})),
			React.DOM.div( {className:"content", dangerouslySetInnerHTML:{__html: this.props.model.get( 'post_content' )}}),
			 comments 
			)
			);
		}
	...
	var PostList = React.createBackboneClass({
		render: function() {
			var posts = this.props.model.map( function( model ) {
				return React.DOM.div( {className:"post", key:model.get('cid')}, PostView( {model:model} ));
			});
